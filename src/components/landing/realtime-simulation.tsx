            ...
            rightElement={
                others.length > 0 && (
                    <div className="flex items-center gap-1 transition-all duration-500 ease-in-out">
                        {others.map((claim) => (
                            <div key={claim.memberId} className="relative animate-in zoom-in slide-in-from-bottom-2 duration-500 fade-in">
                                <RoomMemberAvatar
                                    id={claim.memberId}
...
export function RealtimeSimulation() {
    ...
    const items: ReceiptItemDto[] = [
        { receiptItemId: 'sim-1', quantity: 1, interpretedText: "Short Stack Pancakes", price: 14.00, rawText: "" },
        { receiptItemId: 'sim-2', quantity: 1, interpretedText: "Cold Brew Coffee", price: 5.50, rawText: "" },
    ]
    ...
    const states = [
        ...
        { 'sim-1': [], 'sim-2': [{ memberId: '3', displayName: 'Patricio', avatarUrl: null, quantity: 1 }] },
        // Step 2: Rory grabs the Pancakes
        { 'sim-1': [{ memberId: '2', displayName: 'Rory', avatarUrl: null, quantity: 1 }], 'sim-2': [{ memberId: '3', displayName: 'Patricio', avatarUrl: null, quantity: 1 }] },
    ...
    return (
        <div className="w-full max-w-sm bg-background  z-10 overflow-hidden [&>*]:rounded-t-[inherit] rounded-lg shadow-sm select-none">
            ...
            <CollaborativeRoomHeader
                ...
                activeFilterId={null}
                members={[
                    { roomMemberId: '1', displayName: 'You', avatarUrl: null, isGuest: false },
                    { roomMemberId: '2', displayName: 'Rory', avatarUrl: null, isGuest: false },
                    { roomMemberId: '3', displayName: 'Patricio', avatarUrl: null, isGuest: false },
                ]}
            />
            ...
            <div className="p-2 space-y-2 bg-muted/5 min-h-[140px] transition-all">
                {items.map(item => (
                    <SimulatedItemCard
                        ...
                        item={item}
                        others={currentClaims[item.receiptItemId as keyof typeof currentClaims] as MockClaim[]}
                    />
                ))}
            </div>
