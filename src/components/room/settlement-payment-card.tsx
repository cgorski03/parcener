...
import { AlertCircle, CheckCircle2, Loader2, Trash, Wallet, Plus } from "lucide-react";
import { VenmoButton } from "./venmo-button";
import { Link } from "@tanstack/react-router";
import { usePaymentMethods } from "@/hooks/use-payment-methods";
import { useUpdateRoomPaymentMethod, useGetRoomPulse } from "@/hooks/use-room";
...
export function SettlementPaymentCard({ roomId, isHost, myTotalOwed }: SettlementPaymentCardProps) {
    ...
    if (isHost) {
        const defaultMethod = myPaymentMethods?.find(pm => pm.isDefault) || myPaymentMethods?.[0];

        const containerClasses = "p-4 shadow-sm min-h-[90px] flex flex-col justify-center";
        ...
        if (room.hostPaymentInformation) {
            ...
            return (
                <Card className={`bg-primary/5 border-primary/20 relative ${containerClasses}`}>
                    <div className="flex w-full items-center justify-between gap-4">
                        <div className="flex items-center gap-3">
                            <div className="bg-primary/10 p-2 rounded-full text-primary shrink-0 flex items-center justify-center">
                                {isUnlinking ? <Loader2 className="size-4 animate-spin" /> : <CheckCircle2 className="size-4" />}
                            </div>
                            <div className="flex flex-col gap-0.5">
                                <h4 className="font-semibold text-sm leading-none">Collecting Payments</h4>
                                <p className="text-xs text-muted-foreground leading-none">
                                    Linked via <span className="font-medium text-foreground">{room.hostPaymentInformation.type}</span> to <span className="font-medium text-foreground">@{room.hostPaymentInformation.handle}</span>
                                </p>
                            </div>
                        </div>
        ...
        // --- CASE B: MISSING INFO ---
        if (defaultMethod) {
            const isLinkingThis = isPending && (variables as any)?.paymentMethodId === defaultMethod.paymentMethodId;

            return (
                <Card className={`border-dashed border-destructive/30 bg-destructive/5 ${containerClasses}`}>
                    <div className="flex w-full items-center justify-between gap-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div className="flex flex-col gap-0.5">
                                <h4 className="font-semibold text-sm text-destructive/90 leading-none">Missing Payment Info</h4>
                                <p className="text-xs text-muted-foreground leading-none">
            ...
            <Card className={`border-dashed bg-muted/20 ${containerClasses}`}>
                <div className="flex w-full items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col gap-0.5">
                            <h4 className="font-medium text-sm leading-none">Setup Payments</h4>
                            <p className="text-xs text-muted-foreground leading-none">Link Venmo to settle up.</p>
                        </div>
                    </div>
                    <Button asChild size="sm" variant="outline" className="h-9 text-xs shrink-0 bg-background/50">
    ...
    return (
        <Card className="bg-muted/40 p-4 border-dashed flex flex-col items-center justify-center text-center gap-2 min-h-[90px]">
            ...
            <div className="space-y-1">
                <h4 className="font-medium text-sm text-muted-foreground">Waiting on Host</h4>
