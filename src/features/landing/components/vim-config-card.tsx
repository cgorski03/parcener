      ...
      setCursor((prev) => {
        ...
        if (mode === 'INSERT') {
          ...
          if (e.key.length === 1 && !e.ctrlKey && !e.metaKey && !e.altKey) {
            const newLine =
              currentLine.slice(0, col) + e.key + currentLine.slice(col);
            const newLines = [...lines];
            newLines[row] = newLine;
            setLines(newLines);
            return { row, col: col + 1 };
        ...
        switch (e.key) {
          ...
          case 'l':
            return clamp(row, col + 1, 'NORMAL', lines);
          case 'j':
            return clamp(row + 1, col, 'NORMAL', lines);
          case 'k':
            return clamp(row - 1, col, 'NORMAL', lines);
          case '$':
            return clamp(row, Infinity, 'NORMAL', lines);
          case '0':
            return { row, col: 0 };
          case 'i':
            setMode('INSERT');
    ...
    >
      <div className="space-y-4 relative z-10 pointer-events-none">
        ...
        <p className="text-slate-400 text-sm leading-relaxed">
          {' '}
          Instant, affordable scalability.
        </p>
      </div>
      ...
      >
          ...
          <div className="flex">
            ...
            <div className="flex-1 overflow-x-auto whitespace-pre font-medium">
              {lines.map((line, rowIndex) => (
                <div key={rowIndex} className="relative h-[15px]">
                  {' '}
                  ...
                  {line.split('').map((char, colIndex) => {
                    ...
                    if (line.startsWith('#')) colorClass = 'text-slate-500';
                      ...
                      (line.includes('"') &&
                        ...
                        !line.startsWith('compat') &&
                        !line.startsWith('DB'))
                    ) {
                      // Crude string detection
                      colorClass = 'text-green-400';
                    } else if (
                      ['name', 'compatibility_date', 'DB_URL'].some((k) =>
                        line.startsWith(k),
                      ) &&
                      colIndex < line.indexOf('=')
                    ) {
        ...
        <div className="rounded-b-md bg-slate-700 text-[10px] font-mono flex items-center shadow-xl border-x border-b border-slate-700 select-none">
          ...
          <div className="px-2 text-slate-400 flex-1 truncate text-center">
            parcener/wrangler.toml
          </div>
