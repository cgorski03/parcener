...
import type { FullRoomInfoDto, ReceiptItemDto } from '@/shared/dto/types';
...
export function useSettlementCalculation(room: FullRoomInfoDto | undefined) {
  return useMemo(() => {
    ...
    const memberClaimedItems = new Map<
      string,
      UserSettlement['claimedItems']
    >();
    ...
    claims.forEach((claim) => {
      const item = receipt.items.find(
        (i) => i.receiptItemId === claim.receiptItemId,
      );
      if (!item) return;
