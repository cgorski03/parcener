...
describe('edit-receipt-service', () => {
  describe('editReceiptItem', () => {
    it('updates receipt item', async () => {
      ...
      const { items } = await createSuccessfulReceipt(
        user.id,
    ...
    it('will adjust quantity of second claimer when reducing quantity', async () => {
      ...
      const seededRoom = await createTestRoom(
        testDb,
        seededReceipt.id,
        user1.id,
      );
      const member1 = await createTestRoomMember(testDb, seededRoom.id, {
        userId: user1.id,
      });
      const member2 = await createTestRoomMember(testDb, seededRoom.id, {
        userId: user2.id,
      });
      const firstClaim = await createTestClaim(
        testDb,
        seededRoom.id,
        member1.id,
        items[0].id,
      ...
      const secondClaim = await createTestClaim(
        testDb,
        seededRoom.id,
        member2.id,
        items[0].id,
      ...
      expect(claims.find((c) => c.id === firstClaim.id)?.quantity).toBe(
        (2).toFixed(2),
      );
      expect(claims.find((c) => c.id === secondClaim.id)?.quantity).toBe(
        (1).toFixed(2),
      );
    });
  });
