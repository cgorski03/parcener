...
export async function pruneExcessClaimsHelper(db: DbType | DbTxType, receiptItemId: string, newMaxQuantity: number) {
    ...
    const totalClaimed = existingClaims.reduce((sum, c) => sum + Number(c.quantity), 0);
