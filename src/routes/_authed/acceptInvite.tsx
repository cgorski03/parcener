...
export const Route = createFileRoute('/_authed/acceptInvite')({
  ...
  head: () => ({
    meta: [
      ...
      { property: 'og:title', content: 'Accept Invitation | Parcener' },
      {
        ...
        content: 'Accept invitation to begin uploading receipts!',
      },
    ],
  }),
  loaderDeps: ({ search }) => ({
  ...
  loader: async ({ deps, context }) => {
    ...
    try {
      const result = await acceptInviteRpc({ data: { token: deps.token } });

      // Clear user cache so subsequent navigation fetches fresh canUpload status
      if (result.status === 'SUCCESS') {
        context.queryClient.removeQueries({ queryKey: ['user'] });
      }
