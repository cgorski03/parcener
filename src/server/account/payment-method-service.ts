...
import { AppUser, DbType, user as userTable, paymentMethod } from "../db";
import { CreatePaymentMethodRequest, PaymentMethodDto } from "../dtos";
import { mapPaymentMethodToDto } from "../dto-mappers";
...
export async function getUserPaymentMethods(
    ...
    const returnedUser = await db.query.user.findFirst({
        where: eq(userTable.id, requestingUser.id),
        with: {
            paymentMethods: true,
        },
    });
