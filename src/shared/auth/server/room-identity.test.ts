...
import { parseRoomIdentity } from './room-identity';

describe('room-identity', () => {
    describe('parseRoomIdentity', () => {
        it('returns user identity when user is authenticated', () => {
            ...
            const result = parseRoomIdentity(req, roomId, user);
        ...
        it('returns guest identity when guest cookie is present', () => {
        ...
        it('returns both user and guest identity when both are present', () => {
            const guestUuid = '550e8400-e29b-41d4-a716-4466554400000';
            const req = new Request('http://localhost', {
                headers: { cookie: `guest_uuid_room_room456=${guestUuid}` },
            });
        ...
        it('returns null identity when neither user nor guest is present', () => {
            const req = new Request('http://localhost', {
                headers: { cookie: '' },
            });
            const user = undefined;
        ...
        it('lowercases guest UUID from cookie', () => {
            const guestUuid = '550e8400-e29b-41d4-a716-4466554400000';
            const req = new Request('http://localhost', {
                headers: { cookie: `guest_uuid_room_room456=${guestUuid}` },
            });
        ...
        it('extracts guest UUID from cookie string with other cookies', () => {
            const guestUuid = '550e8400-e29b-41d4-a716-4466554400000';
            const req = new Request('http://localhost', {
                headers: {
