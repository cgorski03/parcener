...
import type { CreateReceiptItemDto, ReceiptItemDto, ReceiptTotalsDto } from '@/shared/dto/types'
import { ReceiptWithRoom } from '@/features/receipt-review/server/get-receipt-service'
import { createReceiptItemRpc, deleteReceiptItemRpc, editReceiptItemRpc, finalizeReceiptTotalsRpc } from '@/features/receipt-review/server/rpc-put-receipt'
...
const updateReceiptCache = (queryClient: any, receiptId: string, updater: (old: ReceiptWithRoom) => ReceiptWithRoom) => {
...
export function useCreateReceiptItem(roomId: string | null) {
    ...
    return useMutation({
        ...
        mutationFn: async (args: { receiptId: string; item: CreateReceiptItemDto }) => {
        ...
        onError: (_, { receiptId }, context) => {
            if (context?.previousData) {
                queryClient.setQueryData(ReceiptQueryKeys.detail(receiptId), context.previousData)
            }
        },
        onSettled: (_, __, { receiptId }) => {
            queryClient.invalidateQueries({ queryKey: ReceiptQueryKeys.detail(receiptId) })
            queryClient.invalidateQueries({ queryKey: ReceiptQueryKeys.valid(receiptId) })
            if (roomId) queryClient.invalidateQueries({ queryKey: RoomQueryKeys.detail(roomId) })
        },
    })
}
